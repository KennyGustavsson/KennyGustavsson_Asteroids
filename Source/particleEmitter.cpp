...
FollowingParticleEmitter::FollowingParticleEmitter(int maxParticles, vector2* Pos, vector2* Rot, vector2* screenSize)
{
	...
	for (int i = 0; i < _maxParticle; i++)
	{
		...
		if (_useRot) 
		{
			...
			_particles[i] = new Particle(pos, rot, life, _color, _screenSize);
		}	
		else 
		{
...
FollowingParticleEmitter::FollowingParticleEmitter(int maxParticles, vector2* Pos, SDL_Color color, float spawnTime, float randomColor, vector2* screenSize)
{
	_pos = Pos;
	_maxParticle = maxParticles;
	_particles = std::vector<Particle*>(_maxParticle);
	...
	for (int i = 0; i < _maxParticle; i++) {
		if (_particles[i]->isDead()) {
			if (_timer > _spawnTime || _spawnTime == 0.0f) {
				...
				_color.r - Random(_randomColor),
				_color.g - Random(_randomColor),
				_color.b - Random(_randomColor),
				255 };
				...
				if (_useRot)
				{
					...
					_particles[i]->SetColor(particleColor);
